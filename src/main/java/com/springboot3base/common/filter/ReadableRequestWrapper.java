...
@Slf4j
public class ReadableRequestWrapper extends HttpServletRequestWrapper { // Actually, we don't need this class
    ...
    public ReadableRequestWrapper(HttpServletRequest request) {
        super(request);
        this.params.putAll(request.getParameterMap()); // original parameter
        ...
        try {
            ...
            if (StringUtils.isEmpty(collect)) { // remove empty content
                is.close();
                return;
            }
